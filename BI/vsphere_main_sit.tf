  ...
  customize {
    ...
    linux_options {
     ...
     network_interface {
      ...
      dns_suffix_list = local.dns_suffix
  }
}

  provisioner "remote-exec" {
    script = "scripts_post_conf/part_second_disk.sh"
    connection {
...
resource "vsphere_virtual_machine" "vms_with_shared_disk" {

for_each = var.vms_with_shared_disk
...
#Digunakan untuk disk shared
disk {
    
    size = each.value.shared_disk_size
    label = each.value.shared_disk_label
    unit_number = 2
  }
...
clone {
  ...
  provisioner "remote-exec" {
    scripts = ["scripts_post_conf/part_second_disk.sh", "scripts_post_conf/part_shared_disk.sh"]
    connection {
      host = each.value.ipv4_data
      type = "ssh"
      agent = false
