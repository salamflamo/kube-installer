...
# Fase 1 Untuk memprovisioning 3 VM dengan 2 disk 
...
count = var.vm2-count
name = "${var.vm-name}${4 + count.index}"
resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
datastore_id = data.vsphere_datastore.datastore.id
num_cpus = var.vm-cpu
memory = var.vm-ram
...
disk {
  label = "${var.vm-name}${4 + count.index}-disk"
   size             = data.vsphere_virtual_machine.template.disks.0.size
   eagerly_scrub    = data.vsphere_virtual_machine.template.disks.0.eagerly_scrub
   thin_provisioned = data.vsphere_virtual_machine.template.disks.0.thin_provisioned
}
...
disk {
    size = 1
    name = "${var.vm-name}${4 + count.index}_2.vmdk"
    unit_number = 1
  }

disk {
    size = 3
    name = "${var.vm-name}${4 + count.index}_3.vmdk"
    unit_number = 2
  }
...
clone {
  ...
  customize {
    ...
    linux_options {
      host_name = "ansible${4 + count.index}"
      domain = var.vm-domain
    }
    
     network_interface {
        ipv4_address = "192.168.234.${111 + count.index}"
        ipv4_netmask = "25"
      }
